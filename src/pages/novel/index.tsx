...
const withOptionStoryIndex = [StoryIndex.Story2, StoryIndex.Story3, StoryIndex.Story5];
...
export default function Novel({}: Props) {
  ...
  const withFooter = !data?.withOption || (data?.withOption && data?.selected != Option.None);

  const onClickOption = (option: Option) => {
    // 선택지 업데이트하고 결과 전송하고
    const beforeOptions = JSON.parse(window.localStorage.getItem('xletter_option'));
    ...
    window.localStorage.setItem('xletter_option', JSON.stringify(beforeOptions));
      ...
      <div className={styles.novel_container}>
        <div className={styles.novel}>
          ...
          <div className={styles.novel_content_container}>
            <Content storyIndex={storyIndex} data={data} onSelectOption={onClickOption} />
          </div>
        </div>
      </div>
      {withFooter && (
        <NovelFooter isLastStory={storyIndex === StoryIndex.Story5} getStoryNext={getStoryNext} goSurvey={goSurvey} />
      )}
    </div>
  );
}
